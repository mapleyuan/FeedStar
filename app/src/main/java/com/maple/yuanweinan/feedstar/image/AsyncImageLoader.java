...
public class AsyncImageLoader {
	...
	/**
	 * 构造方法, 必须在UI线程调用
	 * @param context 建议传ApplicationContex
	 * @param imageCache
	 */
	...
	public void removeAllTask() {
		mLocalThreadPool.shutDown();
		mNetThreadPool.shutDown();
	}
...
//		LogUtils.d("matt", LogUtils.getStackTraceString(new Throwable()));
		...
		//从内存读取
		Bitmap result = loadImgFromMemery(request.getImageSavePath(), request.mImageUrl);
	 ...
	 */
	protected void handleLoadImage(final ImageLoadRequest request) {
			...
			if (isNetTask) {
				mNetThreadPool.submit(new PriorityRunnable(request) {
					...
					public void run() {
						...
						Bitmap bmp = loadImgFromMemery(request.getImageSavePath(), request.mImageUrl);
						if (bmp == null || bmp.isRecycled()) {
							//通过图片加载器来获取图片
							bmp = loader.loadImage(request);
							//保存到缓存
							if (request.mIsCache) {
								saveImgToMemory(bmp, request.getImageSavePath(), request.mImageUrl);
							}
						}
	 ...
	 * @return
	 */
	protected final Bitmap loadImgFromMemery(String imgFullPath, String imgUrl) {
		String key = getImgKeyInMemory(imgFullPath, imgUrl);
		if (null == key) {
			return null;
		}
	 ...
	 * @param imgUrl
	 */
	private void saveImgToMemory(Bitmap bitmap, String imgFullPath, String imgUrl) {
		if (null == bitmap) {
			return;
		}
		String key = getImgKeyInMemory(imgFullPath, imgUrl);
	 ...
	 * @return
	 */
	private String getImgKeyInMemory(String imgFullPath, String imgUrl) {
	...
	public static class ImageLoadRequest {
		...
		public AsyncNetBitmapOperator mNetOperator;
		/** 图片加载结果回调, 回调在UI线程上执行 */
		...
		public ImageLoadRequest(String imageUrl, String imagePath, String imageName) {
			mImageUrl = imageUrl;
