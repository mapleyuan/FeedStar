...
import com.maple.yuanweinan.feedstar.utils.FileUtils;
...
public class NetImageLoader implements ImageLoaderItf {
	...
	public Bitmap loadImage(AsyncImageLoader.ImageLoadRequest request) {
		...
		if (!FileUtils.isSDCardAvaiable()) {
			...
			return com.maple.yuanweinan.feedstar.image.SdImageLoader.loadImgFromSD(imgSavePath, request.mScaleCfg);
		} else {
	...
	public static boolean saveNetImgToSD(String imgUrl, String savePath) {
		...
		try {
			...
			result = FileUtils.saveInputStreamToSDFile(inputStream, savePath);
